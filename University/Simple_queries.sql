...
/* 1.3 */
SELECT DISTINCT Teacher_student_group.TeacherPersonalNumber, Teacher_student_group.AudienceNumber
FROM Teacher_student_group
WHERE Teacher_student_group.SubjectCodeNumber = '18Ï';

/* 1.4 */
SELECT DISTINCT Teacher_student_group.SubjectCodeNumber, Subject.Name
FROM Teacher_student_group
JOIN Subject
...
/* 1.5 */
SELECT Teacher_student_group.GroupCodeNumber
FROM Teacher_student_group
JOIN Teacher
...
/* 1.9 */
SELECT Subject.Name AS 'Subject name', Student_group.Name AS 'Group name'
FROM Subject, Student_group
JOIN Teacher_student_group
